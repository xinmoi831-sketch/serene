...
const collections = {
  ...
  sessions: new Datastore({ filename: path.join(dbPath, "sessions.db"), autoload: true }),
  // Long-term memory store — one document per user
  memory:   new Datastore({ filename: path.join(dbPath, "memory.db"),   autoload: true }),
};

collections.users.ensureIndex({ fieldName: "email", unique: true });
collections.messages.ensureIndex({ fieldName: "userId" });
collections.messages.ensureIndex({ fieldName: "createdAt" });
...
collections.sessions.ensureIndex({ fieldName: "userId" });
collections.sessions.ensureIndex({ fieldName: "date" });
collections.memory.ensureIndex({ fieldName: "userId", unique: true });

collections.payments      = new Datastore({ filename: path.join(dbPath, "payments.db"),      autoload: true });
collections.therapists    = new Datastore({ filename: path.join(dbPath, "therapists.db"),    autoload: true });
collections.bookings      = new Datastore({ filename: path.join(dbPath, "bookings.db"),      autoload: true });
collections.conversations = new Datastore({ filename: path.join(dbPath, "conversations.db"), autoload: true });

collections.conversations.ensureIndex({ fieldName: "userId" });
collections.conversations.ensureIndex({ fieldName: "updatedAt" });

collections.hotlines = new Datastore({ filename: path.join(dbPath, "hotlines.db"), autoload: true });
collections.hotlines.ensureIndex({ fieldName: "is_active" });
